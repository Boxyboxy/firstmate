...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Codex, OpenCode, Cursor Agent CLI, or omp.
...
| `/bearings`        | Generate a decision-ready chat digest - position, Captain's Call with evidence and a recommendation, Recently Landed, Underway with risks, and a ready-first Charted Next - from bounded local fleet and registered-secondmate state, for the whole fleet or the projects you name; use `/bearings file` to also replace today's dated report in `data/`, and add `include PRs` when live PR enrichment is wanted |
...
- `/bearings` returns the fresh fleet-wide digest in chat only.
- `/bearings <project>` (or "where are we on <project>") scopes the same digest to one or more registered projects and keeps the fleet-integrity warnings.

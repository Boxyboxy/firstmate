...
- `bin/fm-session-lock-lib.sh`: the `FM_HARNESS_RE` process-name pattern (anchor it, e.g. `^omp$`, to avoid substring collisions), so the primary/secondmate can find its harness PID and hold the session lock instead of running read-only forever.
...
- `bin/fm-busy-lib.sh`: the harness's semantic busy source and its trust gate, so its own lifecycle records classify the task instead of a rendered-text heuristic.
- `bin/fm-composer-lib.sh`: any needed `FM_COMPOSER_IDLE_RE` empty-composer override, plus any novel composer shape, bare agent prompt glyph, or idle placeholder, taught to `fm_composer_classify_screen` only. It is the ONE fleet-wide owner of every composer shape and the `empty`/`pending`/`pending-unproven`/`unknown` decision, so teaching it there gives every backend the shape in the same commit, no adapter may carry its own copy, and a new harness's own idle composer is not misread as a dead shell.
...
- `docs/sessionstart-nudge.md`: the session-start tier, transport, and source routing for the new harness.
...
`omp` also blocks directly, but through a tracked auto-discovered `.omp/extensions/fm-primary-turnend-guard.ts` whose `session_stop` handler returns `{ continue: true, additionalContext }` when the shared guard exits 2 (a return-value block, not an exit-2 hook process).
...
| omp | `--model <model>` | `--thinking <low\|medium\|high\|xhigh\|max>` | Re-verified 2026-07-31 on omp (Oh My Pi) 17.2.2; `omp --help` advertises `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, and `auto`. omp also accepts off/minimal/auto outside Firstmate's shared effort axis. |
...
## omp (VERIFIED 2026-07-13, omp 16.4.8; core re-verified 2026-07-27 on omp 17.1.5; semantic busy lifecycle verified 2026-07-31 on omp 17.2.1 - pane process name `omp`, launch flags `--auto-approve`/`--model`/`--thinking`/`-e`/`--no-session`, `OMPCODE=1` in tool subprocesses, and the `session_stop` forced-continuation contract; launch flags, the `--no-extensions` suppression boundary, the lifecycle event set with its deferred `ctx.isIdle()` edge, the `tool_call` block contract, and the `session_stop` forced continuation re-verified 2026-07-31 on omp 17.2.2 in print mode, and the process name `omp` re-verified there too; `OMPCODE=1` and interactive-pane busy sampling not re-checked since 17.2.1, and interrupt/exit/`--resume`/trust-dialog not re-checked since 16.4.8; subagent root-session attribution, the per-session factory call, the main-session-only `session_stop`, the interrupt path's `agent_end` without `session_stop`, and interactive-pane busy sampling verified 2026-09-05 on omp 18.1.10)

omp (Oh My Pi) is Pi-derived - it shares Pi's `-e/--extension` wiring, `pi.on(...)` API, `--thinking` effort axis, model roles, and `~/.omp/agent/` sessions - so `pi` is the closest launch template.
But omp sets BOTH `OMPCODE=1` and `CLAUDECODE=1` (detection checks `OMPCODE` first), has a real approval system (`--auto-approve`), a named `omp` pane process (so secondmate liveness is confident), a native `.omp/extensions/*.ts` auto-discovery root, and a `session_stop` hook that blocks a turn by RETURNING `{ continue: true, additionalContext }` - a third turn-end class (direct-blocking-via-return-value), distinct from claude/codex exit-2 blocking and pi/pi-signed/opencode passive follow-up.

omp's lifecycle event set diverges from Pi's in two ways that matter for busy state, so do not assume the Pi extension body transfers unchanged.
...
| Busy state | The Firstmate-owned extension's `omp-ext` records: `agent_start` (busy), and `agent_end` re-checked shortly afterwards and written idle only when `ctx.isIdle()` is then true. omp never emits `agent_settled`, and `ctx.isIdle()` reads false at the instant `agent_end` fires, so Pi's synchronous `agent_settled` guard cannot be reused verbatim; the deferred re-check preserves the same protection against auto-retries, compaction, tool loops, and queued continuations. Only the root session's instance writes: a `task` subagent's `agent_start` and `agent_end` write nothing, while its `turn_end` still touches the notification marker. | live extension probes on 17.2.1, print mode and interactive tmux pane over two turns; subagent attribution on 18.1.10 by `FM_OMP_LIVE_E2E=1 tests/fm-omp-busy-subagent-live-e2e.test.sh` |
| Delivery-guard busy token | `⟦esc⟧` (interrupt hint; present during thinking `⠧ Working… ⟦esc⟧` and tool `⠇ <verb> ⟦esc⟧`; absent when idle). Fixed byte string; matches under LC_ALL=C and UTF-8. omp's `Working…` uses U+2026 ellipsis, so the existing `Working\.\.\.` token does NOT match omp - `⟦esc⟧` is the only reliable token. This token is NOT a task-state source: it serves only `bin/fm-tmux-lib.sh`'s submit acknowledgement and away-mode pane guard. | captured pane, both phases |
...
| Role models | Global `modelRoles` remains authoritative for `smol`, `slow`, `plan`, `task`, `commit`, `tiny`, `designer`, `advisor`, and `vision` during a positional launch, including when `--model` and `--thinking` select the parent model, so Firstmate does not pass `--smol`, `--slow`, or `--plan` and roles without CLI flags stay on the same map too. | omp 17.2.2 docs plus live JSON event streams on 2026-08-01: an interactive launch resolved bundled `task` to `openai-codex/gpt-5.6-sol:xhigh`; a separate `-p --mode json --no-session` launch resolved parent `openai-codex/gpt-5.6-sol` and bundled `scout` through `@smol` to `openai-codex/gpt-5.6-luna:high` |
| Prewalk | Firstmate does not pass `--prewalk` or `--no-prewalk`, so the captain's global `prewalk.enabled` setting remains authoritative because enabling it on an implementation spawn would hand the parent from its selected implementation model to the `smol` role after the first edit while a scout has no edit boundary. | omp 17.2.2 `--help`, settings schema, and prewalk runtime docs |
...
| Idle composer | rounded box, NO ghost/placeholder text when idle; the shared `fm-composer-lib.sh` classifier handles it (no `FM_COMPOSER_IDLE_RE` override) | styled idle capture |
| Background-notify watcher | `bin/fm-watch-arm.sh` run as an async background job returns one of `watcher: started`, `watcher: attached`, or `watcher: FAILED` (`bin/fm-watch-arm.sh` owns that contract) and integrates with omp's async-job/`job poll` model (same class as claude/grok). Start that job with the bash tool's `timeout` set to `0`: the arm parks indefinitely on a quiet fleet, and omp's default command deadline would kill the wake delivery path while the watcher child itself stays healthy. | live arm during this build; deadline kill observed live 2026-07-31 on omp 17.2.2 |
...
omp's help says explicit `-e` paths still work with `--no-extensions`, but live probes found that `--no-extensions` suppressed both explicit `-e` and `--hook` files - first on 16.4.8, and re-verified unchanged on 17.2.2 (2026-07-31): the same extension loaded and printed its marker under `-e` and under `--hook`, and printed nothing when either was combined with `--no-extensions`.
...
1. Delivery-guard busy token: launch omp on a long task, capture the pane; confirm `⟦esc⟧` still appears while busy AND is absent when idle. Patch `bin/fm-composer-lib.sh` `FM_DELIVERY_OMP_BUSY_REGEX_DEFAULT` (the per-harness token `fm_busy_lines_match` selects for the submit acknowledgement and away-mode pane guard, not for task state), and its `⟦esc⟧` share of `FM_DELIVERY_BUSY_REGEX_DEFAULT`.
...
4. Launch flags and role isolation: `omp --help` for `--auto-approve`, `--model`, `--thinking`, `--smol`, `--slow`, `--plan`, `--prewalk`, and `-e`; run a positional launch with task progress enabled and confirm the parent profile still leaves global task/smol role resolution intact. Patch `bin/fm-spawn.sh` if a flag is renamed or the role-isolation contract changes.
...
5b. Subagent attribution (LOAD + ATTRIBUTE): run `FM_OMP_LIVE_E2E=1 tests/fm-omp-busy-subagent-live-e2e.test.sh`; it launches the exact `fm-spawn` command in a real pane, fans out one real `task` subagent, and fails naming the omp version if a child session's completion reaches the root's busy record or the root's own completion no longer settles it. Patch `bin/fm-spawn.sh`'s omp arm if omp stops calling the factory per session, starts firing `session_stop` for subagents, or exposes a direct parent/child field to prefer.

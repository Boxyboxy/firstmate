...
| omp | `--model <model>` | `--thinking <low\|medium\|high\|xhigh\|max>` | Re-verified 2026-07-31 on omp (Oh My Pi) 17.2.1; `omp --help` advertises `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, and `auto`. omp also accepts off/minimal/auto outside Firstmate's shared effort axis. |

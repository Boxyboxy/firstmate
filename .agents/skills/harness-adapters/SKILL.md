...
- `bin/fm-watch.sh` and `bin/fm-tmux-lib.sh`: the busy-signature default (keep the two defaults byte-identical), plus any needed `FM_COMPOSER_IDLE_RE` empty-composer override and any novel bare agent prompt glyph in `bin/fm-composer-lib.sh`'s shared composer classifier.
...
omp runs `bin/fm-watch-arm.sh` as its own native async background job and treats the job completion as the wake, with no generated bridge because omp's primary turn-end guard auto-discovers from the tracked `.omp/extensions/`.

...
- `bin/fm-composer-lib.sh`: any needed `FM_COMPOSER_IDLE_RE` empty-composer override, plus any novel bare agent prompt glyph in the shared composer classifier (the one fleet-wide owner of the empty/dead-shell/pending decision, so a new harness's own idle composer is not misread as a dead shell).
...
| omp | Run `omp models`, the subcommand omp's own help documents for listing, searching, and refreshing available models; `--model` fuzzy-matches an alias, bare name, or `provider/model` form. |
...
omp's lifecycle event set diverges from Pi's in one way that matters for busy state, so do not assume the Pi extension body transfers unchanged.
omp never emits `agent_settled`; per turn it emits `agent_start`, then `turn_end`, `session_stop`, and `agent_end`.
...
| Runtime bound | `--max-time=<duration>` accepts bare seconds, `<n>m`, and `<n>h`, and ends the run when it elapses. Firstmate passes `--max-time=3h` on every omp crewmate and secondmate launch; `config/omp-max-time` overrides that per home, and `off` passes no bound. `bin/fm-spawn.sh` `omp_max_time_flag()` and [`docs/configuration.md`](../../../docs/configuration.md#harness-support) own the config contract. | `omp --help` plus the 2026-08-02 duration sample: healthy tasks finished in 20-60 minutes, one converge ran 9h24m until manual intervention |
...
1. Delivery-guard busy token: launch omp on a long task, capture the pane; confirm `⟦esc⟧` still appears while busy AND is absent when idle. Patch `bin/fm-tmux-lib.sh` `FM_TMUX_OMP_BUSY_REGEX_DEFAULT` (the per-harness token `fm_busy_lines_match` selects for the submit acknowledgement and away-mode pane guard, not for task state).
2. Env marker: `env | grep OMPCODE`; patch `bin/fm-harness.sh` if renamed.
3. Process name: run the real-harness liveness drift guard (`FM_HARNESS_LIVENESS_DRIFT=1 bin/fm-test-run.sh tests/fm-harness-liveness-drift-live-e2e.test.sh`), which enumerates omp and asserts the exact `omp` path component the classifier depends on; patch `bin/backends/tmux.sh` + `bin/fm-session-lock-lib.sh` `FM_HARNESS_RE`/`FM_HARNESS_NAMES` if it changed.
4. Launch flags and role isolation: `omp --help` for `--auto-approve`, `--model`, `--thinking`, `--max-time`, `--smol`, `--slow`, `--plan`, `--prewalk`, and `-e`; confirm `--max-time` still accepts bare seconds, `<n>m`, and `<n>h`; run a positional launch with task progress enabled and confirm the parent profile still leaves global task/smol role resolution intact. Patch `bin/fm-spawn.sh` if a flag is renamed or the role-isolation contract changes.

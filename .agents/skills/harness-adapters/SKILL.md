...
omp runs `bin/fm-watch-arm.sh` as its own native async background job and treats the job completion as the wake, with no generated bridge (unlike Pi) because omp's primary turn-end guard auto-discovers from the tracked `.omp/extensions/`.

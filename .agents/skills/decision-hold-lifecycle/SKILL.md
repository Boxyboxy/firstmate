...
A hold records what one worker observed on the day it was filed, so re-verify it against current code and its initiative's authoritative planning source, per `AGENTS.md` section 6, before relaying it to the captain or briefing work from it.
A hold that source contradicts, or that shipped work has already answered, is stale: correct or close it rather than presenting it, and never carry an unverified premise into a brief.
...
5. Before relaying any hold, re-verify its premise as above, and close or correct the stale ones instead of presenting them.
...
7. If the captain authorizes dependent work, record it with normal tasks-axi commands and block it by the hold identity.
8. Put the captain's exact durable decision in a file and close the hold with the script's `resolve` command and every routed task, its `decline` command when the answer routes no work, or its `repair` command when the hold was already closed outside the script.
9. Confirm Bearings no longer shows the closed hold and that any routed work remains in structured backlog state.

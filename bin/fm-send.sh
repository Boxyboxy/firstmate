...
# Usage: fm-send.sh <target> [--resolve-key <key>]... [--allow-long] <text...>
...
# decisions. --allow-long, given after the target and after any --resolve-key
# flags, sends the long text anyway for the rare deliberate case, and is the
# only opt-out: the cap is a local constant on purpose, not an FM_SEND_*
# environment tunable. The refusal is text-only: --key sends, key sequences,
# and submit verification are untouched.

...
`bin/fm-test-run.sh` is the single owner of behavior-suite selection, portable CI lane composition, optional local `--jobs` for the proven-isolated set only, the hard per-script bound, per-script stdin isolation, per-script timing markers, family totals, the coverage guard, and the optional JSON timing artifact.

...
| OMP | Nudge | `.omp/extensions/fm-primary-turnend-guard.ts`, auto-discovered from the tracked `.omp/extensions/` root, handles `session_start` reasons `startup`, `new`, and `resume`, then injects the wrapper's nudge with the Pi-derived `pi.sendMessage`. | Native message injection is supported through the auto-discovered extension. omp stays on the nudge tier because Pi's run-tier promotion has not been verified against an omp session, so the marked nudge makes the session run `bin/fm-session-start.sh` itself. |
...
Pi is the only adapter that injects a run-tier digest as a message rather than as hook stdout, so whatever it injects must carry operational provenance or the Ahoy skill would have to guess whether it was captain-authored.
...
omp's extension injects only the already-encoded nudge, so it needs neither that encoding step nor that bound.
...
It also proves each hook-config harness registers its own tier's wrapper exactly once, and that the tracked omp extension delivers exactly the wrapper's nudge bytes as a non-displayed custom message for `startup`, `new`, and `resume` and nothing for any other reason.

...
| OMP | `.omp/extensions/fm-primary-turnend-guard.ts` handles `session_start` reasons `startup`, `new`, and `resume`, then injects the wrapper output with the Pi-derived `pi.sendMessage` API. | OMP 16.4.8 already verified that its committed auto-discovered extension receives `session_start`; tracked registration is covered by `tests/fm-sessionstart-nudge.test.sh`. |
...
### Claude, Pi, and OMP wiring smoke checks
...
The same registration test verifies that OMP's tracked `session_start` handler invokes the wrapper and uses the Pi-derived context-message API.
...
It also verifies tracked wrapper registration for Claude, Codex, OpenCode, Pi, OMP, and Grok.

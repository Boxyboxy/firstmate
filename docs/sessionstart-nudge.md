...
| --- | --- | --- |
| Run | Executes `bin/fm-session-start.sh` in the hook and lets its ordered digest land in model context before the first turn. | Claude, `codex exec`, Pi / pi-signed, Cursor, OMP |
...
| OMP | Run | `.omp/extensions/fm-primary-turnend-guard.ts` maps omp's verified `session_start` reasons `startup`, `new`, and `resume` onto wrapper sources, handles `session_compact` as the compaction equivalent, and injects the output with the same Pi-derived `pi.sendMessage` API. | Native context injection is supported through the committed auto-discovered extension. omp's reason vocabulary is verified only for those three, so `fork` stays unmapped rather than guessed. |

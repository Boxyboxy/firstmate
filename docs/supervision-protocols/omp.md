...
9. If a forced restart is genuinely needed, run `bin/fm-watch-arm.sh --restart` through the same omp background async job mechanism and treat `watcher: healthy ...` as proof that the replacement cycle is live.

...
8. When the background async job completes with `signal:`, `stale:`, `check:`, or `heartbeat`, drain queued wakes, handle them, then start exactly one fresh background async job on the same deadline-disabled terms.
9. If a forced restart is genuinely needed, run `bin/fm-watch-arm.sh --restart` through the same deadline-disabled omp background async job mechanism and treat its `watcher: started ...` line as proof that the replacement cycle is live.

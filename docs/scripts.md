...
| `fm-bearings-snapshot.sh` | Project the fleet snapshot to the compact TOON bearings view, with `repo` and `ready` on every project row; `--repo` scopes it to named projects; local-only unless `--include-prs` |
...
| `fm-omp-update.sh`       | Replace the machine-wide `omp` executable through its own resolved channel once every local worker is confirmed stopped; `--check` is detect-only |
...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, proven-isolated `--jobs`, hard per-script bound, stdin isolation, coverage guard, timing/JSON |
...
| `fm-base-lib.sh`         | Single owner of the admissible task-base shape, shared by the brief and the spawn    |
...
| `fm-pr-merge.sh`         | Record PR metadata, then merge a task's canonical full GitHub or GitLab URL, refusing a pull request with failing checks |
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode |
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees, require completed scout deliverables, retire secondmate homes |
| `fm-housekeeping.sh`     | Reclaim disk left by finished crew work while protecting live work and captain-owned stacks |

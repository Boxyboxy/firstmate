...
| `fm-brief.sh`            | Scaffold ship briefs with explicit `--mode`, plus scout, secondmate-charter, and Herdr-lab briefs with absolute firstmate paths |
...
| `fm-classify-lib.sh`     | Shared captain-relevant and declared-external-wait wake classification vocabulary, durable keyed-decision folds, and scans |
...
| `fm-pr-merge.sh`         | Record PR metadata, then merge a task's canonical full GitHub URL, refusing a pull request with failing checks |

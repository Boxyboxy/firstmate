...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok and omp use background-notify cycles, Codex uses bounded foreground checkpoints, Pi uses its two tracked primary extensions, and OpenCode uses its TUI plugin.

...
FM_OMP_HANDOFF_MAX_RECORDS=32   # most pending actionable wakes the omp session-replacement handoff retains, oldest dropped first (.omp/extensions/fm-primary-omp-watch.ts "Handoff retention")
FM_OMP_HANDOFF_MAX_AGE_MS=86400000   # age in milliseconds past which an omp session-replacement handoff wake is dropped instead of replayed

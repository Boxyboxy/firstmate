...
The recognized harness names are `claude`, `codex`, `opencode`, and `grok` matched as substrings, plus an exact `omp`: the first four were confirmed to run as their own literal process name with `ps -ef` on 2026-07-07 (`claude`, `codex`, and `opencode` are each a native compiled binary per `file` reporting Mach-O, so their `comm` is their own binary name with no interpreter wrapper to hide behind), and `omp` was re-verified as the exact pane process name on omp 16.4.8 on 2026-07-13.

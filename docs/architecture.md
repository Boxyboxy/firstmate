...
PR-based task merges go through `bin/fm-pr-merge.sh`, which classifies the PR's check rollup as structured data and refuses a red or unreadable one - AGENTS.md's "Never merge a red PR" rule enforced mechanically at the single merge path - before recording `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` and calling `gh pr merge`.
A captain-authorized red merge is the only exception, and it is recorded in the task's meta before the merge so the decision stays durable.
...
It calls `gh` directly rather than the `gh-axi` wrapper because it pins the merge to the exact head commit whose checks it classified, and the wrapper rebuilds its `gh` argv from a fixed flag allowlist that would drop that pin silently; the script's own header owns the rest of the merge-safety contract.

...
`--allow-red-checks` is the captain-authorized exception, and it records `merge_checks_override=<reason>` in the task's metadata before merging so the decision stays durable.

...
The per-adapter semantic sources behind [`bin/fm-busy-lib.sh`](../../bin/fm-busy-lib.sh) were live-verified on 2026-07-28, and omp on 2026-07-31, against firstmate-launched workers wired exactly as `fm-spawn` writes them.
...
omp does not reuse Pi's `agent_settled` edge, and the disconfirming evidence is what forced that.
A probe extension registering `agent_start`, `agent_settled`, `turn_end`, `agent_end`, `session_start`, and `session_stop` was loaded with `-e` on omp 17.2.1, in `--print` mode and in an interactive pane over two full turns.
`agent_settled` registered without error but never fired in any run, while each turn produced `agent_start`, `turn_end`, `session_stop`, and `agent_end`.
Deferred re-reads showed `ctx.isIdle()` false at the instant `turn_end`, `session_stop`, and `agent_end` fire and true within 250ms after, which is why the idle write is deferred and re-checked rather than taken synchronously.
The same probe was re-run on omp 17.2.2 on 2026-07-31 in `--print` mode (`omp -p --auto-approve --no-session -e <probe>`) and reproduced that event set exactly: `session_start` with `isIdle=true`, then `agent_start`, `turn_end`, `session_stop`, and `agent_end` all with `isIdle=false`, then `isIdle=true` on a 250ms deferred re-read, and no `agent_settled` in any run.
The interactive-pane two-turn sampling above was not re-run on 17.2.2.
...
The blocking and bounded-follow-up mechanisms were validated across seven harnesses on 2026-07-08 through 2026-08-13, with Claude's replacement Stop-owned path revalidated on 2026-07-24, Grok's adaptive `Stop` path revalidated on 2026-07-28, and Cursor's stop-hook park validated on 2026-08-13.
...
| OMP | 16.4.8; re-verified 17.2.2 on 2026-07-31 | Direct-blocking `session_stop` return value with an 8-continuation cap | The async handler forced one continuation through `{ continue: true, additionalContext }`, auto-discovered from the tracked `.omp/extensions/` root, and its one-shot flag plus the built-in cap bounded it. The 17.2.2 re-run in `--print` mode forced exactly one continuation whose reply came from the injected `additionalContext`, then saw `session_stop` again with the one-shot flag already spent and did not block a second time. `tool_call` was re-verified in the same pass: `event.type`/`event.toolName`/`event.input.command` still arrive as the guard reads them, and returning `{ block: true, reason }` refused a `bash` call with that exact reason instead of running it. |

...
`tests/fm-omp-harness.test.sh` covers the omp replacement handoff's retention rules: a stored wake whose every named task has lost its `state/<id>.meta` is dropped at load and never delivered, identical undelivered wakes collapse to one however many arm closes minted them, the record bound and age cut-off drop the oldest first, and a live task's undelivered wake still survives a session replacement and is replayed.

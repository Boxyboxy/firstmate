...
# FM_PI_HARNESS (Pi family), GROK_AGENT (grok).
# codex and opencode have no env markers (ancestry only). Without this, a local
# claude/omp/pi/grok session fails cases that pin a different fake harness while
# CI (no ambient markers) still passes. OMPCODE has to be cleared even on the Pi
# path: omp sets it alongside CLAUDECODE and wins the Layer-1 detection order.
run_session_start() {

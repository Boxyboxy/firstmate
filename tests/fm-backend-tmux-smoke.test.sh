...
# Wait for the shell's observable completion marker instead of assuming the
# command will finish inside a fixed sleep on a loaded full-suite runner.
i=0
while [ "$i" -lt 50 ]; do
...
case "$out" in
  *tag-line-80*) : ;;
  *) fail "numbered output did not complete before the capture assertion"$'\n'"$out" ;;
esac

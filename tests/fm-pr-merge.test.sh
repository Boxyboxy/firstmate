...
#   (t) a GitLab MR URL resolves and merges through glab instead of erroring
#   (u) glab is addressed by the host from the URL, never an assumed one
#   (v) no merge method is imposed on GitLab, so the project's own one applies
#   (w) each pre-merge condition refuses independently, and all of them report
#   (x) a stale recorded pr_head= is reported and the live head is verified
#   (y) an unreadable merge request state refuses rather than merging blind
#   (z) glab or jq absent refuses before any state is recorded
#   (aa) --sha in extra GitLab args fails fast, and still forwards on GitHub
#   (ab) a GitLab refusal still leaves pr= recorded and the merge poll armed
set -u

...
When a change must not land before a matching change elsewhere, record that order as a `blocked-by` edge between the two work items: `bin/fm-pr-merge.sh` then refuses the dependent merge, naming what is missing, until the blocker is closed against its landed PR.

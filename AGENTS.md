...
Write a ship task's base as `origin/<branch>`, because its PR target and its local-only merge target are derived from it; a bare branch name, another remote's ref, a tag, or a raw commit is refused rather than resolved against a ref `git fetch origin` never refreshes.
A `local-only` task must be based on the project's default branch, since the guarded local merge fast-forwards that branch and nothing else.

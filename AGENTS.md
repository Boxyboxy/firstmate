...
`bin/fm-pr-merge.sh` enforces that default itself and refuses a check state it cannot read, so an explicit captain instruction is carried out through its `--allow-red-checks` flag, which records the authorized reason in the task's meta, never by reaching around the guard.
